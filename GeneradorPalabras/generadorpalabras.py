...
def find_vowel_distance(vocal,G_DistanciaMaximaVocal,G_DistanciaFonemasVocales):
    """
    Función que encuentra las vocales que se pueden cambiar por la vocal dada dentro de un rango de distancia fonética.

    Args:
        vocal: La vocal que se va a cambiar.
        G_DistanciaMaximaVocal: Rango máximo de distancia fonética para el cambio de vocal.
        G_DistanciaFonemasVocales: Matriz con las distancias fonéticas entre las vocales del español.

    Returns:
        Lista de vocales que se pueden cambiar por la vocal dada dentro del rango de distancia fonética especificado.
    """
    letras_candidatas = []
...
def matriz_distancias(matriz):  # Matriz generada al principio del programa
    """
    Función que genera un diccionario con las distancias entre las consonantes del español.

    Args:
        matriz: Matriz que contiene la información fonética de las consonantes del español.

    Returns:
        Diccionario con las distancias entre las consonantes del español.
    """
...
def getDistanciaConsonante(silaba_referencia,silaba_modificada,dificultad,distancias_consonantes,G_ListaFonemasConsonantes):
    """
    Función que calcula la distancia fonética entre dos sílabas en función de las consonantes que contienen.

    Args:
        silaba_referencia: La sílaba de la palabra original.
        silaba_modificada: La sílaba a la que se le ha aplicado un cambio.
        dificultad: Nivel de dificultad para la generación de palabras.
        distancias_consonantes: Matriz de distancias entre consonantes.
        G_ListaFonemasConsonantes: Lista de fonemas consonánticos.

    Returns:
        True si la distancia fonética entre las dos sílabas está dentro del rango de dificultad especificado, False en caso contrario.
    """
...
def getDistanciaTotal(palabra_referencia,palabra_modificada,distancias_consonantes,distancias_vocales,G_ListaFonemasConsonantes,G_ListaVocales):
    """
    Calcula la distancia fonética total entre dos palabras.

    Parameters:
        palabra_referencia (str): Palabra de referencia.
        palabra_modificada (str): Palabra que se quiere calcular la distancia.
        distancias_consonantes (dict): Matriz de distancias entre los fonemas consonánticos.
        distancias_vocales (dict): Matriz de distancias entre los fonemas vocálicos.
        G_ListaFonemasConsonantes (list): Lista de los fonemas consonánticos.
        G_ListaVocales (list): Lista de los fonemas vocálicos.

    Returns:
        int: Distancia fonética total entre las dos palabras.
    """
...
def palabrasCandidatas(lista_palabras,G_Palabras_Corco):
    """
    Encuentra las palabras de una lista que están presentes en otra lista.

    Parameters:
        lista_palabras (list): Lista de palabras que se van a comprobar.
        G_Palabras_Corco (list): Lista de palabras en las que se va a buscar.

    Returns:
        list: Lista de palabras que están presentes tanto en lista_palabras como en G_Palabras_Corco.
    """
...
def ordeno_foneticamente(lista_ordenar,palabra_original,C_DistanciasEntreConsonantes,G_DistanciaFonemasVocales,G_ListaFonemasConsonantes,G_ListaVocales):
    """
    Ordena una lista de palabras fonéticamente.

    Parameters:
        lista_ordenar (list): Lista de palabras que se van a ordenar.
        palabra_original (str): Palabra original de entrada.
        C_DistanciasEntreConsonantes (matrix): Matriz de distancias entre fonemas consonánticos.
        G_DistanciaFonemasVocales (matrix): Matriz de distancias entre fonemas vocálicos.
        G_ListaFonemasConsonantes (list): Lista de fonemas consonánticos.
        G_ListaVocales (list): Lista de fonemas vocálicos.

    Returns:
        list: Lista de palabras ordenadas fonéticamente.
    """
...
def buscar_palabra_acento(G_Palabras_sin_Tilde,G_Palabras_Corco, G_Acento_Palabra,palabra, palabra_semilla,G_DIC_TILDE,tilde_original):
    """
    Función que busca la forma con tilde de una palabra en el diccionario G_Palabras_Corco.

    Args:
        G_Palabras_sin_Tilde: Diccionario que mapea palabras sin tilde a sus formas canónicas.
        G_Palabras_Corco: Diccionario que mapea palabras con tilde a sus formas canónicas.
        G_Acento_Palabra: Diccionario que mapea palabras a la posición de la sílaba tónica.
        palabra: La palabra a la que se le quiere añadir la tilde.
        palabra_semilla: La palabra original que se ha introducido.
        G_DIC_TILDE: Diccionario que mapea palabras a una lista de posibles tildes.
        tilde_original: Booleano que indica si la palabra original tenía tilde.

    Returns:
        La forma con tilde de la palabra, o la palabra original si no se encuentra en el diccionario.
    """
...
def add_accent(G_Palabras_sin_Tilde, G_Palabras_Corco, G_Acento_Palabra,listado_candidatas, INPUT_PALABRA,G_DIC_TILDE,tilde_original):
    """
    Función que añade el tilde a una lista de palabras candidatas.

    Args:
        G_Palabras_sin_Tilde: Diccionario que mapea palabras sin tilde a sus formas canónicas.
        G_Palabras_Corco: Diccionario que mapea palabras con tilde a sus formas canónicas.
        G_Acento_Palabra: Diccionario que mapea palabras a la posición de la sílaba tónica.
        listado_candidatas: Lista de palabras candidatas a las que se les quiere añadir el tilde.
        INPUT_PALABRA: La palabra original que se ha introducido.
        G_DIC_TILDE: Diccionario que mapea palabras a una lista de posibles tildes.
        tilde_original: Booleano que indica si la palabra original tenía tilde.

    Returns:
        Lista de palabras con tilde.
    """
    l_accent = []
    ...
    if tilde_original:
        for l_a, l_o in zip(l_accent,l_original):
            # Si la palabra original coincide con la palabra acentuada, añadir la original a la lista de palabras sin modificar
            if l_a == l_o:
...
def genera_palabras(INPUT_PALABRA, INPUT_TIPO_CAMBIO, G_DistanciaMaximaVocal, G_DistanciaMaximaConsonante,G_PSEUDO,G_LOG):
    '''''
    ...
    if (len(listado_candidatas) > 0):
        if G_LOG != 0:
            ...
            if INPUT_TIPO_CAMBIO not in [3,4,6,7]:
                ...
                print(f"\t[*] Palabras de salida : {OUTPUT_PALABRA}")
